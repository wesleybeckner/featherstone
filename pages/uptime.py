...
master["Duration"] = (master["Accountable To Date"] - \
master["Accountable From Date"]).dt.total_seconds()/60/60
...
def select_data(df,
    ...
    new_df = new_df.loc[(new_df["Accountable From Date"].dt.year >= year[0]) &
                        (new_df["Accountable To Date"].dt.year <= year[1])]
...
def make_bar_plot(occurences=300, reason='Stop Category', year=[2017,2020]):
    new_df = select_data(master,
                        occurences=occurences,
                        reason=reason,
                        year=year)
        ...
        html.Div([
           ...
           html.P(
            ...
            dcc.RangeSlider(id='uptime_year_slider',
                        min=min(master["Accountable From Date"]).year,
                        max=max(master["Accountable To Date"]).year,
                        value=[min(master["Accountable From Date"]).year,
                            max(master["Accountable To Date"]).year],
                        marks={'{}'.format(i): '{}'.format(i) for i in\
                         master["Accountable From Date"].dt.year.unique()},
                         className="dcc_control")
                         ],
