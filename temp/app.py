...
def display_page(pathname):
    if pathname == "/featherstone/rate":
    ...
    elif pathname == "/featherstone/full-view":
        return (
        ...
        return animate.create_layout(app)
...
def display_hover_data(groupby, year, metric, distClickData, barClickData):
    return rate.make_performance_plot(groupby, year, metric, distClickData, barClickData)
@app.callback(
...
def display_hover_data(groupby, year, metric, clickData):
    return rate.make_pareto_plot(groupby, year, metric, clickData)
@app.callback(
...
def update_bar_plot(year, metric):
    return rate.make_bar_plot(year, metric)
